...
<<echo=false>>=
cat.df.without.rownames(starmine[row.names(starmine) %in% c(23134, 23135, 23136, 17849, 23143, 23144, 51835),
                                 c("date", "name", "fwd.ret.1m", "fwd.ret.6m","smi")]
)
@ 
...
bt <- backtest(starmine, id.var = "id",
               ...
               ret.var = "fwd.ret.1m", natural = TRUE, by.period = FALSE)
@
...
<<sector backtest>>=
bt <- backtest(starmine, in.var = "smi", ret.var = "fwd.ret.1m", by.var = "sector", by.period = FALSE)
@ 
...
bt <- backtest(starmine, in.var = "smi", ret.var = "fwd.ret.1m", by.var = "cap.usd", buckets = c(5, 10), by.period = FALSE)
@ 
...
bt <- backtest(starmine, in.var = "smi", buckets = 4,
               ret.var = c("fwd.ret.1m", "fwd.ret.6m"), by.period = FALSE)
@ 
